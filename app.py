...
if st.session_state.step == 1:
    ...
    with col2:
        ai_play = st.text_input("🎨 AI玩法", placeholder="客户具体要求、风格描述等，如：文生视频，治愈系文字独白，心情感受，整体风格文艺治愈")
...
# ============================================================
    ...
    if st.session_state.mode == "流星":
        ...
        if _meteor_recommended:
            ...
            # LLM可能没按格式输出【最终推荐】，fallback：取最后一条
            _meteor_display_text = _inspiration_items[-1]["title"] + "\n" + _inspiration_items[-1]["content"] if _inspiration_items else inspiration_text
        st.markdown(f'<div class="result-box">{_meteor_display_text}</div>', unsafe_allow_html=True)
        # 流星模式只展示推荐条，覆盖_inspiration_items用于后续勾选逻辑
...
# ============================================================
    ...
    if "generated_prompts" not in st.session_state or not st.session_state.generated_prompts:
        ...
        # 策划级别：负面风格约束只调1次LLM，缓存到session_state
        if "negative_style_hint" not in st.session_state:
            try:
                from modules.prompt_writer import get_negative_style_hint
                st.session_state.negative_style_hint = get_negative_style_hint(ui["theme"], ui["ai_play"])
            except Exception:
                # LLM失败时本地兜底
                st.session_state.negative_style_hint = "不要恐怖风格、怪诞风格、压抑冰冷风格，要温暖治愈、积极向上的可爱风格"

        _negative_hint = st.session_state.negative_style_hint
        ...
        for shot in st.session_state.storyboard:
            ...
            # 注入负面风格约束（策划级别，只调了1次LLM）
            if _negative_hint and result["静态"]:
                from modules.prompt_writer import _inject_negative_hint
                result["静态"] = _inject_negative_hint(result["静态"], _negative_hint)
    ...
    for i, p in enumerate(st.session_state.generated_prompts):
        ...
        # 可编辑的静态prompt（用text_input替代text_area，规避Streamlit Cloud交互bug）
        new_static = st.text_input(
            "静态图 Prompt",
            value=p["静态提示词"],
        ...
        # 复制提示词按钮（使用JS实现一键复制）
        _copy_text = combined.replace("'", "\\'").replace("\n", "\\n")
        st.markdown(f"""
        <button onclick="navigator.clipboard.writeText('{_copy_text}').then(function(){{var b=this;b.textContent='已复制!';setTimeout(function(){{b.textContent='复制提示词';}},1500);}}.bind(this));" 
