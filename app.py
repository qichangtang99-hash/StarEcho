...
if "splash_shown" not in st.session_state:
    ...
    # 启动弹窗：先展示简单动画，封面图用st.image延迟加载（不走base64）
    _splash_cover_p = _static_path("cover.png")
    _splash_placeholder = st.empty()
    with _splash_placeholder.container():
        # 用st.image渲染封面，Streamlit自己处理图片传输
        if _splash_cover_p:
            st.image(_splash_cover_p, use_container_width=True)
        st.markdown('''
        <div style="text-align:center;">
            ...
            <div style="margin-top:14px;height:3px;background:rgba(120,160,220,0.15);border-radius:3px;overflow:hidden;">
                <div style="height:100%;background:linear-gradient(90deg,#5080c0,#90b8f8);border-radius:3px;animation:splashLoading 2s ease-in-out forwards;"></div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    # 延迟清空启动弹窗
...
def _static_path(filename):
    """返回static目录下文件的绝对路径（供st.image使用，不走base64）"""
    p = os.path.join(_static_dir, filename)
    return p if os.path.exists(p) else None

with st.sidebar:
    # Logo — 用st.image直接渲染，不走base64嵌入
    _logo_p = _static_path("logo.png")
    if _logo_p:
        st.image(_logo_p, width=80)
    st.markdown("### 响星")
    st.caption("AI彩铃策划Agent")
    st.divider()
    st.markdown("**响星导航，星盘指路**")
    # 功能图标行 — 用st.image渲染小图标
    _func_items = [
        ("icons/01_引星自定_透明背景.png", "引星自定"),
        ("icons/02_星图不迷_透明背景.png", "星图不迷"),
        ("icons/03_流星天降_透明背景.png", "流星天降"),
        ("icons/04_暗礁不触_透明背景.png", "暗礁不触"),
        ("icons/05_星火点亮_透明背景.png", "星火点亮"),
    ]
    _icon_cols = st.columns(len(_func_items))
    for i, (_icon_file, _func_name) in enumerate(_func_items):
        with _icon_cols[i]:
            _ip = _static_path(_icon_file)
            if _ip:
                st.image(_ip, width=32)
            st.caption(_func_name)
    st.divider()
